...
if (process.env.HTTPS === 'TRUE') {
	let options = {};
	if (process.env.TLS_KEY == null || process.env.TLS_KEY.trim().length === 0) {
		options = await getTls();
	} else {
		options.key = fs.readFileSync(process.env.TLS_KEY, 'utf8');
		options.cert = fs.readFileSync(process.env.TLS_CRT, 'utf8');
	}
